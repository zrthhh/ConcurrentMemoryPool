...
public:
	...
	{
		assert((k >> BITS) == 0);    //k的范围必须在[0, 2^BITS-1]中
		array_[k] = v;    //建立映射
	}
private:
...
public:
	...
	void* get(Number k)const
	{
		...
		return root_[i1]->values[i2];    //返回该页号对应的span指针
	}

	void set(Number k, void* v)
	{
		const Number i1 = k >> LEAF_BITS;    //计算第一层的下标
		const Number i2 = k & (LEAF_LENGTH - 1);    //计算第二层的下标
		assert(i1 < ROOT_LENGTH);
		root_[i1]->values[i2] = v;    //建立该页号与对应的span的映射
	}
	...
	bool Ensure(Number start, size_t n)
	{
		for (Number key = start; key <= start + n - 1;)
		{
			...
			if (root_[i1] == NULL)    //第一层i1下标指向的空间未有开辟
			{
				//开辟对应的空间
...
public:
	...
	bool Ensure(Number start, size_t n)
	{
		for (Number key = start; key <= start + n - 1;)
		{
			...
			if (root_->ptrs[i1] == NULL)//第一层i1下标指向空间未开辟
			{
				//开辟对应空间
				Node* n = NewNode();
